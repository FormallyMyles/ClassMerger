...
public class Tools {
	...
	private static String CACHE_DIR = System.getProperty("java.io.tmpdir") + File.separatorChar + "agentcache.0.0_" + REV;
	...
	private static void unpack(String path) {
		try {
			URL url = Tools.class.getClassLoader().getResource(NATIVE_DIR + ((Platform.is64Bit() || Platform.getPlatform() == Platform.MAC) ? "64/" : "32/") + path);
			...
			try {
				addToLibPath(libfile.getParent());
	...
	public static void tranform(Class<? extends ClassFileTransformer> transformer, ClassMerger... toRedefine) throws Exception {
		CACHE_DIR = new File("native/").getAbsolutePath();
		loadAgentLibrary();
		AgentLoader.setRedefine(toRedefine);
		AgentLoader.attachAgentToJVM(Tools.getCurrentPID(), transformer, AgentLoader.class);
	}
}